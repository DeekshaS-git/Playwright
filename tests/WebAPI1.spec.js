...
const loginPayLoad = {
    "userEmail": "deekshasharma@ymail.com",
    "userPassword": "Deeksha@123"
}
let token;
test.beforeAll( async() => {
    const apiContext = await request.newContext(); // loading base URL, headers, API only on proxy
    const loginResponse = await apiContext.post('https://rahulshettyacademy.com/api/ecom/auth/login', 
        {data: loginPayLoad })//200,201
        console.log("Login response status:", loginResponse.status());
        expect(loginResponse.ok()).toBeTruthy();
        //after call, we have to get response body to get token
        const loginResponseJson = await loginResponse.json();//json response in variable
        console.log("Login response JSON:", loginResponseJson);
        // Here loginResponseJson is "object"
...
//let's say test1, test2, test3 are there and we want to execute all at time
// then before all 
